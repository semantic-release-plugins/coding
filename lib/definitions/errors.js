...
Your configuration for the \`releasedLabels\` option is \`${stringify(releasedLabels)}\`.`,
  ...
  EINVALIDCODINGNETURL: () => ({
    message: 'The git repository URL is not a valid GitHub URL.',
    details: `The **semantic-release** \`repositoryUrl\` option must a valid coding.net URL with the format \`<Coding_or_CodingEnterprise_URL>/<owner>/<repo>.git\`.
...
If you are using [GitHub Enterprise](https://enterprise.github.com) please make sure to configure the \`githubUrl\` and \`githubApiPathPrefix\` [options](${linkify(
  ...
  ECODINGNETNOPERMISSION: ({owner, repo}) => ({
    message: `The coding.net token doesn't allow to push on the repository ${owner}/${repo}.`,
    details: `The user associated with the [coding.net token](${linkify(
      'README.md#github-authentication'
    )}) configured in the \`CODING_TOKEN\` or \`CODING_TOKEN\` environment variable must allows to push to the repository ${owner}/${repo}.

Please make sure the coding.net user associated with the token is an [owner](https://help.github.com/articles/permission-levels-for-a-user-account-repository/#owner-access-on-a-repository-owned-by-a-user-account) or a [collaborator](https://help.github.com/articles/permission-levels-for-a-user-account-repository/#collaborator-access-on-a-repository-owned-by-a-user-account) if the reposotory belong to a user account or has [write permissions](https://help.github.com/articles/managing-team-access-to-an-organization-repository) if the repository [belongs to an organization](https://help.github.com/articles/repository-permission-levels-for-an-organization).`,
  }),
  EINVALIDCODINGNETTOKEN: ({owner, repo}) => ({
    message: 'Invalid coding.net token.',
    details: `The [coding.net token](${linkify(
      'README.md#github-authentication'
    )}) configured in the \`CODING_TOKEN\` or \`CODING_TOKEN\` environment variable must be a valid [personal token](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line) allowing to push to the repository ${owner}/${repo}.
...
Please make sure to set the \`GH_TOKEN\` or \`GITHUB_TOKEN\` environment variable in your CI with the exact value of the GitHub personal token.`,
  ...
  ENOCODINGNETTOKEN: ({owner, repo}) => ({
    message: 'No coding.net token specified.',
    details: `A [coding.net personal token](${linkify(
      'README.md#github-authentication'
    )}) must be created and set in the \`GH_TOKEN\` or \`GITHUB_TOKEN\` environment variable on your CI environment.

Please make sure to create a [coding.net personal token](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line) and to set it in the \`CODING_TOKEN\` or \`CODING_TOKEN\` environment variable on your CI environment. The token must allow to push to the repository ${owner}/${repo}.`,
  }),
};
